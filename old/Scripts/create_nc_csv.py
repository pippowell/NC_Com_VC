...
for i in range(len(a['dataset'])):
    ...
    df.to_csv(f"{master_path}EEG_CSV/{directory}/eeg_{directory}_{i}_image{image}_class{label}_subject{subject}.csv", index=False)

with open(f'{master_path}EEG_CSV/{directory}/tar_class_labels.csv', 'w') as f:
    pass

ds_size = len(a['dataset'])

for i in range(0, ds_size):
    path = f'{master_path}EEG_CSV/{directory}/*eeg_{directory}_{i}_*.csv'
    files = glob.glob(path)
    file = files[0]
    with open(file,'r') as f:
        reader = csv.reader(f)
        data = list(reader)[1:]
    with open(f'{master_path}EEG_CSV/{directory}/sam{i}_eeg.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerows(data)
    class_from_filename = re.search(r'(?<=class)(\d+)', file)
    label = class_from_filename.group(1)
    label = [label]
    with open(f'{master_path}EEG_CSV/{directory}/tar_class_labels.csv', 'a', newline='') as f:
        writer = csv.writer(f)
